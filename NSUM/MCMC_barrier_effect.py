...
def loglik_rhok_bar(data_mat, index, n, d, mk, rhok):
    sum_lbeta1 = sum(
        sp.special.betaln(mk * (1 / rhok - 1) + data_mat[:, index], d + (1 - mk) * (1 / rhok - 1) - data_mat[:, index]))
...
def loglik_mK_bar(data_mat, index_k, n, d, mK, rhoK):
    sum_lbeta1 = sum(sp.special.betaln(mK * (1 / rhoK - 1) + data_mat[:, index_k],
                                       d + (1 - mK) * (1 / rhoK - 1) - data_mat[:, index_k]))
...
def mcmc_bar(data_mat, data_dict, iterations, burnin, size=None, mu_prior=(2, 8), sigma_prior=(0, 2), rho_prior=(0, 1),
             NK_tuning=None, d_tuning=None, rho_tuning=None):
    ...
    for i in range(burnin + iterations):
        ...
        for j in range(n):
            ...
            if di_new < max(data_mat[j, :]):
                ...
                ratio = loglik_di_bar(data_mat[j, :], mu, sigma, di_new, mknown, mK, rho) - loglik_di_bar(
                    data_mat[j, :], mu, sigma, di_old, mknown, mK, rho)
        ...
        for j in range(len(indices_k)):
            ...
            ratio = loglik_mK_bar(data_mat, index_this_k, n, d, mK_new, rho[index_this_k]) - loglik_mK_bar(data_mat,
                                                                                                           index_this_k,
                                                                                                           n, d, mK_old,
                                                                                                           rho[
                                                                                                               index_this_k])
        ...
        for j in range(data_mat.shape[1]):
            ...
            ratio = loglik_rhok_bar(data_mat, j, n, d, mk_all[j], rhok_new) - loglik_rhok_bar(data_mat, j, n, d,
                                                                                              mk_all[j], rhok_old)
...
if __name__ == "__main__":
    ...
    y, d = simulate_bar(data, 100)
    print(mcmc_bar(y, data, 100, 50)[0])
